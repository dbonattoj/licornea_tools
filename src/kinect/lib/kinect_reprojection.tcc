...
std::vector<kinect_reprojection::sample<Value>> kinect_reprojection::reproject_ir_to_color_samples(
	...
	for(int ir_y = 0; ir_y < 424; ++ir_y) for(int ir_x = 0; ir_x < 512; ++ir_x) {
		...
		ir_i_z_points.emplace_back(static_cast<float>(ir_z));
	}
	...
	std::vector<sample<Value>> samples(n);
	#pragma omp parallel for
	for(std::ptrdiff_t idx = 0; idx < n; ++idx) {
		sample<Value>& samp = samples[idx];
