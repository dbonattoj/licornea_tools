...
def process_view(x, y):	
	...
	view = datas.view(x, y)
	rectified_view = view.group_view("warped")
		...
		if depth:
			...
			if overwrite_depth or not os.path.isfile(out_rectified_depth_filename):
				if unrotate:
					...
					warp_in_depth_filename = tmp_unrotated_depth.filename
				else:
					warp_in_depth_filename = view.depth_filename()
					call_tool("misc/apply_homography", [
						tmp_homography.filename,
						warp_in_depth_filename,
...
def usage_fail():
	print("usage: {} dataset_parameters.json intrinsics.json R.json homographies.json [unrotate?]\n".format(sys.argv[0]))
	sys.exit(1)
...
if __name__ == '__main__':
	...
	unrotate = False
	if(len(sys.argv) > 5): unrotate = (sys.argv[5] == "unrotate")
