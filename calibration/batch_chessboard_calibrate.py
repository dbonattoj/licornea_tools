...
def generate_extrinsic(i, initial_extrinsic, keep_cors):
	...
	cors_filename = os.path.join(output_directory, "cors_{}.json".format(i))
	extrinsic_filename = os.path.join(output_directory, "extrinsic_{}.json".format(i))

	if not os.path.isfile(cors_filename):
		find_chessboard_result = subprocess.call([
			os.path.join(tools_directory, "calibration/find_chessboard"),
			texture_filename,
			cors_filename,
			"6",
			"8",
	...
	args = [
		os.path.join(tools_directory, "calibration/calibrate_extrinsic"),
		cors_filename,
		intrinsic_filename,
		extrinsic_filename
	]
	if initial_extrinsic is not None:
		initial_extrinsic_filename = os.path.join(output_directory, "initial_extrinsic_{}.json".format(i))
	...
	if result != 0: raise Exception("calibrate_extrinsic failed")
...
def export_camera_params(extrinsics, filename):
	...
	for i in range(index_count):
		...
		camera_param = dict()
		camera_param["name"] = "camera_{}".format(i)
...
if __name__ == '__main__':
	...
	extrinsics = [generate_extrinsic(i, None, True) for i in index_range]
	...
	subprocess.call([
		os.path.join(tools_directory, "calibration/denoise_extrinsic_row"),
	...
	extrinsics_refined = [generate_extrinsic(i, camera_params_de[i], False) for i in index_range]
